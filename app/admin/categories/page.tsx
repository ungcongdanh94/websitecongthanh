    ...
    <main className="container-page py-12">
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black">Danh mục</h1>
          <div className="mt-8">
            <SimpleEntityForm endpoint="/api/admin/categories" label="Danh mục" />
          </div>
        </div>
          ...
          {categories.map((item) => (
            <div key={item.id} className="flex items-center justify-between border-b px-5 py-4 last:border-0">
              ...
              <div className="flex items-center gap-4">
                ...
                <Link href={`/admin/categories/${item.id}`} className="text-sm font-semibold text-brand-700">Sửa</Link>
                <EntityDeleteButton endpoint="/api/admin/categories" id={item.id} name={item.name} />
              </div>
            </div>
          ))}
        </div>

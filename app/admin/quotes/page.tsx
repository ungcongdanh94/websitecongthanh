  ...
  return (
    <main className="container-page py-12">
      ...
      <div className="mt-8 grid gap-5">
          ...
          <article key={quote.id} className="rounded-3xl border bg-white p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-slate-600">{quote.phone} {quote.email ? `· ${quote.email}` : ""}</p>
              </div>
              <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-bold text-brand-800">{quote.status}</span>
            </div>
            ...
            {!!quote.items.length && (
              <div className="mt-4 rounded-2xl bg-slate-50 p-4">
                {quote.items.map((item) => (
                  <div key={item.id}>{item.productName} {item.quantity ? `— ${Number(item.quantity)} ${item.unit || ""}` : ""}</div>
                ))}
              </div>
            )}
        ...
        {!quotes.length && <div className="rounded-3xl border bg-white p-10 text-center text-slate-500">Chưa có yêu cầu báo giá.</div>}
      </div>
    </main>
  );
}
